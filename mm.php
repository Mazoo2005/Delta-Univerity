...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    if ($facultyID == 1001) {
        ...
        if (isset($_POST['add_schedule'])) {
            ...
            $lectureDays = $_POST['lecture_days'] ?? [];
            $lectureTimes = $_POST['lecture_times'] ?? [];
            $sectionDays = $_POST['section_days'] ?? [];
            $sectionTimes = $_POST['section_times'] ?? [];
            $lectureClassRooms = $_POST['lecture_class_rooms'] ?? [];
            $sectionClassRooms = $_POST['section_class_rooms'] ?? [];
            $sectionsPerGroup = (int)($_POST['sections_per_group'] ?? 1);
            $studentsPerSection = (int)($_POST['students_per_section'] ?? 30);
            ...
            if ($conn->ping()) {
                ...
                try {
                    for ($i = 1; $i <= $groupCount; $i++) {
                        $lectureDay = (int)($lectureDays[$i] ?? 1);
                        $lectureTime = trim($lectureTimes[$i] ?? '');
                        $lectureClassRoom = trim($lectureClassRooms[$i] ?? '');

                        if (!empty($lectureTime)) {
                            // تعديل: استخدام عمود Time بدلاً من LectureTime
                            $sql = "INSERT INTO Schedule (CourseID, FacultyID, GroupNumber, SectionNumber, ClassRoom, Time, DAY, SectionsPerGroup, StudentsPerSection) 
                                    VALUES (?, ?, ?, 0, ?, ?, ?, ?, ?)";
                            $stmt = $conn->prepare($sql);
                            if ($stmt === false) {
                                throw new Exception("Prepare failed: " . $conn->error);
                            }
                            $stmt->bind_param('iiisssii', $courseID, $facultyID, $i, $lectureClassRoom, $lectureTime, $lectureDay, $sectionsPerGroup, $studentsPerSection);
                            ...
                            for ($j = 1; $j <= $sectionsPerGroup; $j++) {
                                ...
                                if (!empty($sectionTime)) {
                                    ...
                                    $stmt->bind_param('iiiisssii', $courseID, $facultyID, $i, $j, $sectionClassRoom, $sectionTime, $sectionDay, $sectionsPerGroup, $studentsPerSection);
                    ...
                    if ($success && $rowsInserted > 0) {
                        ...
                        throw new Exception('No valid data provided or no rows inserted');
                    }
                } catch (Exception $e) {
                    $conn->rollback();
                    $response = ['status' => 'error', 'message' => 'Error saving schedule: ' . ($errorMsg ?: $e->getMessage())];
                    error_log("Schedule save error: " . ($errorMsg ?: $e->getMessage()));
                }
            } else {
                $response = ['status' => 'error', 'message' => 'Database connection lost'];
            }
...
if (isset($_GET['edit_course'])) {
    ...
    $result = safeQuery($conn, "SELECT * FROM Schedule WHERE CourseID = ? ORDER BY GroupNumber, SectionNumber", [$editCourseID]);
    while ($row = $result->fetch_assoc()) {
        $currentSchedule[] = $row;
    }
}
...
<body>
                    ...
                    <form method="POST" class="schedule-form" id="schedule-form">
                        ...
                        <div class="section-settings">
                            ...
                            <div class="capacity-summary">
                                <div>Total Sections: <span id="total_sections"><?php echo !empty($currentSchedule) ? count($currentSchedule) : '0'; ?></span></div>
                                <div>Total Student Capacity: <span id="total_students"><?php echo !empty($currentSchedule) ? count($currentSchedule) * $currentSchedule[0]['StudentsPerSection'] : '0'; ?></span></div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="group_count" id="group_count" min="1" max="10" class="form-control" required 
                                   value="<?php echo !empty($currentSchedule) ? count(array_unique(array_column($currentSchedule, 'GroupNumber'))) : '1'; ?>">
                        </div>
                        ...
                        <div id="group_schedules">
                            <?php if (!empty($currentSchedule)): ?>
                                ...
                                foreach ($currentSchedule as $schedule): 
                                    if ($currentGroup != $schedule['GroupNumber']) {
                                        if ($groupIndex > 0) {
                                            echo '</div>';
                                        }
                                        $currentGroup = $schedule['GroupNumber'];
                                        echo '<div class="group-schedule">';
                                        echo "<h4>Group $currentGroup</h4>";
                                        $lectureShown = false;
                                    }

                                    if ($schedule['SectionNumber'] == 0 && !$lectureShown) {
                                        echo '<div class="form-group">';
                                        echo '<label>Lecture Day:</label>';
                                        echo '<select name="lecture_days[' . $currentGroup . ']" class="form-control">';
                                        for ($i = 1; $i <= 7; $i++) {
                                            $dayName = date('l', strtotime("Sunday +{$i} days"));
                                            $selected = ($schedule['DAY'] == $i) ? 'selected' : '';
                                            echo "<option value='$i' $selected>$dayName</option>";
                                        }
                                        echo '</select>';
                                        echo '</div>';
                                        
                                        // تعديل: استخدام Time بدلاً من LectureTime
                                        echo '<div class="form-group">';
                                        echo '<label>Lecture Time:</label>';
                                        echo '<input type="time" name="lecture_times[' . $currentGroup . ']" class="form-control" value="' . htmlspecialchars($schedule['Time']) . '">';
                                        echo '</div>';
                                        
                                        echo '<div class="form-group">';
                                        echo '<label>Lecture Classroom:</label>';
                                        echo '<input type="text" name="lecture_class_rooms[' . $currentGroup . ']" placeholder="Example: Room 101" class="form-control" value="' . htmlspecialchars($schedule['ClassRoom']) . '">';
                                        echo '</div>';
                                        $lectureShown = true;
                                    } elseif ($schedule['SectionNumber'] > 0) {
                                ?>
                                    <div class="form-group">
                                        <label>Section <?php echo $schedule['SectionNumber']; ?> Day:</label>
                                        <select name="section_days[<?php echo $currentGroup; ?>][<?php echo $schedule['SectionNumber']; ?>]" class="form-control">
                                            <?php for ($i = 1; $i <= 7; $i++): ?>
                                                <?php $dayName = date('l', strtotime("Sunday +{$i} days")); ?>
                                                <option value="<?php echo $i; ?>" <?php echo ($schedule['DAY'] == $i) ? 'selected' : ''; ?>>
                                                    <?php echo $dayName; ?>
                                                </option>
                                            <?php endfor; ?>
                                        </select>
                                    ...
                                    <!-- تعديل: استخدام Time بدلاً من SectionTime -->
                                    <div class="form-group">
                                        <label>Section <?php echo $schedule['SectionNumber']; ?> Time:</label>
                                        <input type="time" name="section_times[<?php echo $currentGroup; ?>][<?php echo $schedule['SectionNumber']; ?>]" class="form-control" value="<?php echo htmlspecialchars($schedule['Time'] ?? ''); ?>">
                                    </div>
                                    
                                    <div class="form-group">
                                        <label>Section <?php echo $schedule['SectionNumber']; ?> Classroom:</label>
                                        <input type="text" name="section_class_rooms[<?php echo $currentGroup; ?>][<?php echo $schedule['SectionNumber']; ?>]" placeholder="Example: Room 102" class="form-control" value="<?php echo htmlspecialchars($schedule['ClassRoom']); ?>">
                                    </div>
                                <?php 
                                    }
                                    $groupIndex++;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Dynamic group schedule fields with day and time selection
            ...
            for (let i = 1; i <= count; i++) {
                ...
                let groupHTML = `
                    ...
                    <div class="form-group">
                        ...
                        <select name="lecture_days[${i}]" class="form-control">`;
                
                for (let d = 1; d <= 7; d++) {
                    groupHTML += `<option value="${d}">${daysOfWeek[d-1]}</option>`;
                }
                ...
                groupHTML += `</select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="time" name="lecture_times[${i}]" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label>Lecture Classroom:</label>
                        <input type="text" name="lecture_class_rooms[${i}]" placeholder="Example: Room 101" class="form-control" required>
                    </div>`;

                for (let j = 1; j <= sectionsPerGroup; j++) {
                    groupHTML += `
                        <div class="form-group">
                            <label>Section ${j} Day:</label>
                            <select name="section_days[${i}][${j}]" class="form-control">`;
                    ...
                    groupHTML += `</select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="time" name="section_times[${i}][${j}]" class="form-control">
                        </div>
                        <div class="form-group">
            ...
            notificationForm.addEventListener('submit', function(e) {
                ...
                .then(text => {
                    try {
                        ...
                        showGlobalAlert('error', 'Server returned invalid response: ' + text);
                    }
                })
                .catch(error => {
                    showGlobalAlert('error', 'An error occurred while processing your request: ' + error.message);
                });
            });
        }
        ...
        document.querySelectorAll('form[name="toggle_registration"]').forEach(form => {
            form.addEventListener('submit', function(e) {
                ...
                .catch(error => {
                    showGlobalAlert('error', 'An error occurred while updating registration status: ' + error.message);
                });
            });
        });
        ...
        document.querySelectorAll('form[name="update_courses"]').forEach(form => {
            form.addEventListener('submit', function(e) {
                ...
                .catch(error => {
                    showGlobalAlert('error', 'An error occurred while updating allowed courses: ' + error.message);
                });
            });
        });
        ...
        if (scheduleForm) {
            scheduleForm.addEventListener('submit', function(e) {
                ...
                .catch(error => {
                    showGlobalAlert('error', 'An error occurred while saving schedule: ' + error.message);
                });
            });
        }
